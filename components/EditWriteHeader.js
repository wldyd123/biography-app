...
function EditWriteHeader({onSave, defaultPublic}) {
  ...
  const onNavigateToMyEssay = async () => {
    ...
    const myEssayScreen = navigation
      .getState()
      .routes.find(route => route.name === 'MyEssay'); //'Essay'라는 스크린을 찾아 essayScreen변수에 할당.
    if (myEssayScreen) {
      ...
      navigation.dispatch({
        ...
        source: myEssayScreen.key, //이 액션이 어디서 시작되었는지 지정. essayScreen에서 시작되었음을 나타내기 위함.
      });
    }
  };
    ...
    <View style={styles.block}>
      ...
      <View style={styles.buttons}>
        ...
        <View style={styles.complete}>
          <Pressable onPress={onNavigateToMyEssay}>
